<!-- Responsive Fullscreen Navbar Overlay -->
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div x-show="open" 
        ...
        <button @click="open = false" 
                class="absolute top-6 right-6 p-2 text-gray-500 hover:text-gray-700 focus:outline-none transition duration-150 ease-in-out">
...
<!-- Script to lock scroll when overlay is open -->
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.directive('scroll-lock', (el, { expression }, { effect }) => {
            effect(() => {
                const isOpen = Alpine.evaluate(expression);
                if (isOpen) {
...
<!-- Update the x-data to include scroll locking -->
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Add scroll lock functionality directly without Alpine directive
        ...
        if (navElement) {
            ...
            const observer = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                    if (mutation.type === 'attributes' && mutation.attributeName === 'x-data') {
                        const isOpen = navElement.__x.$data.open;
                        if (isOpen) {
                            document.body.style.overflow = 'hidden';
                        } else {
                            document.body.style.overflow = '';
            ...
            // Simple approach - watch for class changes on the overlay
