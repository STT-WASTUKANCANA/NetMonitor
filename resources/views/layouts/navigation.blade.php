        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center">
                        ...
                        <span class="ml-2 text-xl font-semibold text-gray-800 hidden sm:block">NetMonitor</span>
                    </a>
                </div>
                    ...
                    @can('view alerts')
                        <x-nav-link :href="route('alerts.index')" :active="request()->routeIs('alerts.*')">
                            ...
                            @if(auth()->user()->can('view alerts') && \App\Models\Alert::where('status', 'active')->count() > 0)
                                <span class="ms-1 bg-red-100 text-red-800 text-xs font-medium px-2 py-0.5 rounded-full dark:bg-red-900 dark:text-red-200">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center rounded-full focus:outline-none transition-transform duration-200 hover:scale-105">
                            <x-avatar :user="auth()->user()" size="md" :interactive="true" />
                        </button>
                    </x-slot>
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-3 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
                            <div class="flex items-center">
                                <x-avatar :user="auth()->user()" size="lg" class="mr-3" />
                                <div class="truncate">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('view alerts')
                <x-responsive-nav-link :href="route('alerts.index')" :active="request()->routeIs('alerts.*')">
                    {{ __('Alerts') }}
                    @if(auth()->user()->can('view alerts') && \App\Models\Alert::where('status', 'active')->count() > 0)
                        <span class="ms-2 bg-red-100 text-red-800 text-xs font-medium px-2 py-0.5 rounded-full dark:bg-red-900 dark:text-red-200">
                            {{ \App\Models\Alert::where('status', 'active')->count() }}
                        </span>
                    @endif
                </x-responsive-nav-link>
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            <div class="px-4 flex items-center">
                <x-avatar :user="auth()->user()" size="md" class="mr-3" />
                <div>
