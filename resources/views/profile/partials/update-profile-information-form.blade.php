                ...
                <div class="relative group cursor-pointer">
                    ...
                    @if($user->profile_photo_path)
                        <form method="post" action="{{ route('profile.photo.destroy') }}"
                            ...
                            @method('delete')
                            <button type="submit"
                                class="bg-red-500 hover:bg-red-600 text-white rounded-full p-1 shadow-md transition duration-200"
                    ...
                    <input id="avatar" name="avatar" type="file" accept="image/*"
                        class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onchange="previewAvatar(event)">
                </div>

                <!-- File Info / Instructions -->
                <div class="flex-1">
                    <p class="text-sm text-gray-500">
                        Click the avatar to upload a new photo. Supported formats: JPG, PNG.
                    </p>
                </div>
            </div>
